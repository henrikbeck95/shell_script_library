...
shell_script_library_install() {
    case $(utils_check_if_file_exists "$PATH_LIBRARY_MODULES_FILES_RESULT_ORIGINAL") in
    "false") : ;;
    "true") shell_script_library_uninstall ;;
    esac

    cp "$PATH_LIBRARY_MODULES_FILES_RESULT_ORIGINAL" "$PATH_LIBRARY_RESULT_FILE_INSTALLATION"
}
...
shell_script_library_modules_action_compile_main() {
    local MODULE_ACTION_PLACE="$1"
    local FILENAME_WITH_FULL_PATH
...
shell_script_library_uninstall() {
    ...
    "false") : ;;
    "true") rm "$PATH_LIBRARY_RESULT_FILE_INSTALLATION" ;;
    esac
}
